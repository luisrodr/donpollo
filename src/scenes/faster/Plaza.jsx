...
import { inicialPlaza} from '../../data/makeDataCrud';
...
import {ModalPlaza} from './modal/ModalPlaza';

import { AuthContext } from "../../mycontext";

const URL_BASE=process.env.REACT_APP_URL_BASE;

const API_SEL_SEC=process.env.REACT_APP_API_SEL_SEC;

const API_SEL=process.env.REACT_APP_API_SEL_PLA;
const API_INS=process.env.REACT_APP_API_INS_PLA;
const API_UPD=process.env.REACT_APP_API_UPD_PLA;
const API_DEL=process.env.REACT_APP_API_DEL_PLA;
...
const Plaza = () => {
    ...
    const [inicial, setInicial] = useState(inicialPlaza); 
    ...
    const handleCreateNewRow = (values) => {
          ...
          const idsector=sectorData.find(element=>element.descripcion===values.sector);
          ...
          axios
              ...
              .then((response) => {
                ...
                values.sector=response.data.data.sector;
                console.log("reemplazando sector =========>",values.sector);  
    ...
    const handleEdit= async (values ) => {
        ...
        console.log("id sector ",idsector);
        values.sector={id:idsector.id,descripcion:idsector.descripcion};
   ...
   const handleDeleteRow = useCallback(
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'sector.descripcion',
          header: 'sector',
          size: 140,
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={id: tableData[i].id ,
                   ...
                   sector: tableData[i].sector.descripcion,
                    };
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    ...
                    tableData[i].sector.descripcion,
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialPlaza.title);
   setSubTitle(inicialPlaza.subtitle);
   setIsLoading(true);
   setIsRefetching(true);

   const cargaSector=async()=>{

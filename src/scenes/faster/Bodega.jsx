...
import { inicialBodega} from '../../data/makeDataCrud';
...
const API_SEL=process.env.REACT_APP_API_SEL_BOD;
const API_INS=process.env.REACT_APP_API_INS_BOD;
const API_UPD=process.env.REACT_APP_API_UPD_BOD;
const API_DEL=process.env.REACT_APP_API_DEL_BOD;
...
const Bodega = () => {
    ...
    const [inicial, setInicial] = useState(inicialBodega); 
      ...
      (row) => {
        ...
        if ( re.subfamilia_productos){
            ...
            if  ( re.subfamilia_productos.length >0){
                ...
                setErrorPrueba(true);
                setErrorText(`ID ${row.getValue('id')} no se puede eliminar.subfamilia_productos relacionados...`);
                return;
            };
        };
        //existe elemento
        if ( re.articulos){
          console.log("busqueda de id a borrar",re.articulos); 
          console.log("largo", re.articulos );
        
          if  ( re.articulos.length >0){
              console.log("no se puede borrar");
              setErrorPrueba(true);
              setErrorText(`ID ${row.getValue('id')} no se puede eliminar.articulos relacionados...`);
              return;
          };
        };
 ...
 const csvOptions = {
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialBodega.title);
   setSubTitle(inicialBodega.subtitle);
   setIsLoading(true);
   setIsRefetching(true);

...
import { inicialCliente} from '../../data/makeDataCrud';
...
const API_SCPA=process.env.REACT_APP_API_SEL_CPA;

const API_SEL=process.env.REACT_APP_API_SEL_CLI;
const API_INS=process.env.REACT_APP_API_INS_CLI;
const API_UPD=process.env.REACT_APP_API_UPD_CLI;
const API_DEL=process.env.REACT_APP_API_DEL_CLI;
...
const Cliente = () => {
    ...
    const [inicial, setInicial] = useState(inicialCliente); 
    ...
    const existeCodigo=(code)=>{
       return tableData.find(({sigla }) => sigla === code);
     };
    */
    const handleCreateNewRow = (values) => {

          const idconpag= condicionpagoData.find(element=>element.descripcion===values.condicionpago);
          console.log("id idconpag ",idconpag);   
          ...
          axios
              ...
              .then((response) => {
                ...
                values.condicionpago=response.data.data. condicionpago;
                console.log("reemplazando  condicionpago =========>",values. condicionpago); 
    ...
    const handleEdit= async (values ) => {
      ...
      if (esUnico(tableData,values.sigla,values.id)){
        //console.log("ya existe el codigo ",values.codigo);
        setErrorPrueba(true)
        setErrorText(`ya existe sigla ${values.sigla}`);
        return;
      };
   ...
   const handleDeleteRow = useCallback(
      ...
      (row) => {
        ...
        //const re = getRelacion(row.getValue('id'));
        ...
        /*
        if ( re.plazas){
            console.log("busqueda de id a borrar",re.plazas); 
            console.log("largo", re.plazas);
          
            if  ( re.plazas.length >0){
        ...
        if (
          !window.confirm(`Seguro quiere borrar ${row.getValue('descripcion')}`)
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'condicionpago.descripcion',
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={id:tableData[i].id,
                   ...
                   razonsocial: tableData[i].razonsocial,
                   giro: tableData[i].giro,
                   direccion: tableData[i].direccion,
                   ciudad: tableData[i].ciudad,};
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    tableData[i].rut ,
                    tableData[i].codigo,
                    tableData[i].razonsocial,
                    tableData[i].giro,
                    tableData[i].direccion,
                    tableData[i].ciudad,
       ];
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialCliente.title);
   setSubTitle(inicialCliente.subtitle);
   setIsLoading(true);
   setIsRefetching(true);
  
   const urlapi=`${URL_BASE}${API_SEL}`
   const urlapicpa=`${URL_BASE}${API_SCPA}`
   

   const cargaCondicionPago=async()=>{
  
    await axios
    .get(urlapicpa,{    headers:{
      Authorization:`Bearer  ${token}`
    }})
    .then(({ data }) => {
     
      setCondicionPagoData(data.data);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                    ...
                    giro:row.original.giro,
                    direccion:row.original.direccion,
                    ciudad:row.original.ciudad,
                    ultimaventa:row.original.ultimaventa,
                    ventames:row.original.ventames,
                    telefono:row.original.telefono,
                    email:row.original.email,
                    nombre:row.original.nombre,
                    condicionpago:row.original.condicionpago

...
import { inicialVendedor} from '../../data/makeDataCrud';
import {ModalVendedor} from './modal/ModalVendedor';


import { AuthContext } from "../../mycontext";

const URL_BASE=process.env.REACT_APP_URL_BASE;
const API_SEL=process.env.REACT_APP_API_SEL_VEN;
const API_INS=process.env.REACT_APP_API_INS_VEN;
const API_UPD=process.env.REACT_APP_API_UPD_VEN;
const API_DEL=process.env.REACT_APP_API_DEL_VEN;
...
const Vendedor = () => {
    ...
    const [inicial, setInicial] = useState(inicialVendedor); 
          ...
          axios
              .post(`${URL_BASE}${ API_INS}`, {
                ...
                data: {
                  ...
                  clave: values.clave,
                  descuentolinea:values.descuentolinea,
                  descuentototal:values.descuentototal,
    ...
    const handleEdit= async (values ) => {
      ...
      console.log("modifica=====ok",values);
   ...
   const handleDeleteRow = useCallback(
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'descuentolinea',
          header: 'descuento linea',
          size: 140,
          Cell: ({ cell }) => {
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={codigo: tableData[i].codigo ,
                  ...
                  descuentolinea:tableData[i].descuentolinea,
                  descuentototal:tableData[i].descuentototal ,
                  comision:tableData[i].comision};
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    ...
                    getNomBool(tableData[i].descuentolinea),
                    getNomBool(tableData[i].descuentototal),
                    tableData[i].comision,
       ];
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialVendedor.title);
   setSubTitle(inicialVendedor.subtitle);
   setIsLoading(true);
   setIsRefetching(true);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                  ...
                  clave:  row.original.clave,
                  descuentolinea: row.original.descuentolinea,
                  descuentototal: row.original.descuentototal,
                  comision:row.original.comision,
                };

...
import { inicialTipoDescuento} from '../../data/makeDataCrud';
...
import {ModalTipoDescuento} from './modal/ModalTipoDescuento';
...
const API_SEL_SEC=process.env.REACT_APP_API_SEL_ATD;

const API_SEL=process.env.REACT_APP_API_SEL_TDE;
const API_INS=process.env.REACT_APP_API_INS_TDE;
const API_UPD=process.env.REACT_APP_API_UPD_TDE;
const API_DEL=process.env.REACT_APP_API_DEL_TDE;
...
const TipoDescuento = () => {
    ...
    const [acciontdeData, setAccionTdeData] = useState([]);
  
    const [rowData, setRowData] = useState([]);

    const [inicial, setInicial] = useState(inicialTipoDescuento); 
    ...
    const handleCreateNewRow = (values) => {
          ...
          const idatd=acciontdeData.find(element=>element.descripcion===values.acciontipodescuento);
          console.log("id idatd) ",idatd);
          ...
          axios
              ...
              .then((response) => {
                ...
                values.acciontipodescuento=response.data.data.acciontipodescuento;
                console.log("reemplazando acciontipodescuento =========>",values.acciontipodescuento);  
    ...
    const handleEdit= async (values ) => {
        ...
        console.log("id atd ",idatd);
        values.acciontipodescuento={id:idatd.id,descripcion:idatd.descripcion};
...
*/
   const handleDeleteRow = useCallback(
      ...
      (row) => {
        ...
        //existe elemento
        if ( re.descuentos){
            console.log("busqueda de id a borrar",re.descuentos); 
            console.log("largo", re.descuentos );
          
            if  ( re.descuentos.length >0){
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'acciontipodescuento.descripcion',
          header: 'accion tipo descuento',
          size: 140,
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={id: tableData[i].id ,
                   ...
                   acciontipodescuento: tableData[i].acciontipodescuento.descripcion,
                    };
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    ...
                    tableData[i].acciontipodescuento.descripcion,
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialTipoDescuento.title);
   setSubTitle(inicialTipoDescuento.subtitle);
   setIsLoading(true);
   setIsRefetching(true);

   const cargaAccionTde=async()=>{
  
    await axios
    .get(`${URL_BASE}${API_SEL_SEC}`,{    headers:{
      Authorization:`Bearer  ${token}`
    }})
    .then(({ data }) => {
     
      setAccionTdeData (data.data);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                  ...
                  acciontipodescuento:  row.original.acciontipodescuento,
        ...
        renderTopToolbarCustomActions={() => (
          <Button
            ...
            onClick={() => {
              ...
              setInicial(inicialTipoDescuento);

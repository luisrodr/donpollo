...
import { inicialTerminal} from '../../data/makeDataCrud';
...
import {ModalTerminal} from './modal/ModalTerminal';

import { AuthContext } from "../../mycontext";

const URL_BASE=process.env.REACT_APP_URL_BASE;

const API_SEL_DEP=process.env.REACT_APP_API_SEL_DEP;

const API_SEL=process.env.REACT_APP_API_SEL_TER;
const API_INS=process.env.REACT_APP_API_INS_TER;
const API_UPD=process.env.REACT_APP_API_UPD_TER;
const API_DEL=process.env.REACT_APP_API_DEL_TER;
...
const Terminal = () => {
    ...
    const [inicial, setInicial] = useState(inicialTerminal); 
    ...
    const handleCreateNewRow = (values) => {
          ...
          const iddepartamento=departamentoData.find(element=>element.descripcion===values.departamento);
          ...
          axios
              ...
              .then((response) => {
                ...
                values.departamento=response.data.data.departamento;
                console.log("reemplazando sector =========>",values.departamento);  
    ...
    const handleEdit= async (values ) => {
        ...
        console.log("id departamento ",iddepartamento);
        values.departamento={id:iddepartamento.id,descripcion:iddepartamento.descripcion};
   ...
   const handleDeleteRow = useCallback(
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'argumento',
          header: 'argumento',
          size: 140,

        },                
        {
          accessorKey: 'departamento.descripcion',
          header: 'departamento',
          size: 140,
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={id: tableData[i].id ,
                   ...
                   funcion: tableData[i].funcion,
                   programa: tableData[i].programa,
                   argumento: tableData[i].argumento,
                   departamento: tableData[i].departamento.descripcion,
                    };
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    ...
                    tableData[i].funcion,
                    tableData[i].programa,
                    tableData[i].argumento,
                    tableData[i].departamento.descripcion,
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialTerminal.title);
   setSubTitle(inicialTerminal.subtitle);
   setIsLoading(true);
   setIsRefetching(true);

   const cargaDepartamento=async()=>{
  
    await axios
    .get(`${URL_BASE}${API_SEL_DEP}`,{    headers:{
      Authorization:`Bearer  ${token}`
    }})
    .then(({ data }) => {
     
      setDepartamentoData(data.data);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                  ...
                  funcion:  row.original.funcion,
                  programa:  row.original.programa,
                  argumento:  row.original.argumento,

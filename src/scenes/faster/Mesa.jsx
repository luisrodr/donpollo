...
import { inicialMesa} from '../../data/makeDataCrud';
...
const API_SEL_EME=process.env.REACT_APP_API_SEL_EME;

const API_SEL_PLA=process.env.REACT_APP_API_SEL_PLA;

const API_SEL=process.env.REACT_APP_API_SEL_MES;
const API_INS=process.env.REACT_APP_API_INS_MES;
const API_UPD=process.env.REACT_APP_API_UPD_MES;
const API_DEL=process.env.REACT_APP_API_DEL_MES;
...
const SubFamiliaProducto = () => {
    ...
    const [inicial, setInicial] = useState(inicialMesa); 
    ...
    const handleCreateNewRow = (values) => {
          ...
          const idplaza=plazaData.find(element=>element.descripcion===values.plaza);
          const idestado=estadoData.find(element=>element.descripcion===values.estadomesa);
      
          console.log("id plaza ",idplaza);
          ...
          axios
              ...
              .then((response) => {
                ...
                values.plaza=response.data.data.plaza;
                console.log("reemplazando plaza =========>",values.plaza);  
        
                values.estadomesa=response.data.data.estadomesa;
                console.log("reemplazando estadomesa =========>",values.estadomesa);  
    ...
    const handleEdit= async (values ) => {
        ...
        values.plaza={id:idplaza.id,descripcion:idplaza.descripcion};

        values.estadomesa={id:idestado.id,descripcion:idestado.descripcion};
   ...
   const handleDeleteRow = useCallback(
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'plaza.descripcion',
          header: 'plaza',
          size: 140,

        },
        {
          accessorKey: 'estadomesa.descripcion',
          header: 'estado mesa',
          size: 140,
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={id: tableData[i].id ,
                   ...
                   asientos: tableData[i].asientos,
                   plaza: tableData[i].plaza.descripcion,
                   estadomesa:tableData[i].estadomesa.descripcion,};
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    tableData[i].codigo ,
                    tableData[i].asientos,
                    tableData[i].plaza.descripcion,
                    tableData[i].estadomesa.descripcion
       ];
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialMesa.title);
   setSubTitle(inicialMesa.subtitle);
   setIsLoading(true);
   setIsRefetching(true);

   const cargaEstado=async()=>{
  
    await axios
    .get(`${URL_BASE}${API_SEL_EME}`,{    headers:{
      Authorization:`Bearer  ${token}`
    }})
    .then(({ data }) => {
     
      setEstadoData(data.data);
  ...
  const cargaPLaza=async()=>{
  
    await axios
    .get(`${URL_BASE}${API_SEL_PLA}`,{    headers:{
      Authorization:`Bearer  ${token}`
    }})
    .then(({ data }) => {
     
      setPlazaData(data.data);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                  ...
                  asientos:  row.original.asientos,
                  plaza:  row.original.plaza,
                  estadomesa: row.original.estadomesa,

...
import { inicialImpresora} from '../../data/makeDataCrud';
import {ModalImpresora} from './modal/ModalImpresora';
...
const API_SEL=process.env.REACT_APP_API_SEL_IMP;
const API_INS=process.env.REACT_APP_API_INS_IMP;
const API_UPD=process.env.REACT_APP_API_UPD_IMP;
const API_DEL=process.env.REACT_APP_API_DEL_IMP;
...
const Impresora = () => {
    ...
    const [inicial, setInicial] = useState(inicialImpresora); 
          ...
          axios
              .post(`${URL_BASE}${API_INS}`, {
                ...
                data: {
                  ...
                  habilita: values.habilita,
                  deshabilita: values.deshabilita,
                  comandopre: values.comandopre,
                  comandopos: values.comandopos,
   ...
   const handleDeleteRow = useCallback(
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'habilita',
          header: 'habilita',
          size: 140,

        },
        {
          accessorKey: 'deshabilita',
          header: 'deshabilita',
          size: 140,

        },
        {
          accessorKey: 'comandopre',
          header: 'comandopre',
          size: 140,

        },
        {
          accessorKey: 'comandopos',
          header: 'comandopos',
          size: 140,
 ...
 const csvOptions = {
  ...
  const handleExportData = () => {
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila={codigo: tableData[i].codigo ,
                  ...
                  puerta: tableData[i].puerta,
                  habilita: tableData[i].habilita,
                  deshabilita: tableData[i].deshabilita,
                  comandopre: tableData[i].comandopre,
                  comandopos: tableData[i].comandopos,};
  ...
  const get_data_to_pdf=()=>{
    ...
    for(let i = 0;i<tableData.length;i++) { 
       ...
       const fila=[ tableData[i].id, 
                    ...
                    tableData[i].puerta,
                    tableData[i].habilita,
                    tableData[i].deshabilita,
                    tableData[i].comandopre,
                    tableData[i].comandopos,
       ];
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialImpresora.title);
   setSubTitle(inicialImpresora.subtitle);
   setIsLoading(true);
   setIsRefetching(true);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                  ...
                  puerta: row.original.puerta,
                  habilita: row.original.habilita,
                  deshabilita: row.original.deshabilita,
                  comandopre: row.original.comandopre,
                  comandopos: row.original.comandopos,

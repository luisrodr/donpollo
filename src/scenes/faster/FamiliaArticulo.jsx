...
import { inicialFamiliaArticulo} from '../../data/makeDataCrud';
//import {ModalGen} from './modal/ModalGen';
//import esUnico   from './helpers/esUnico';

import { inicialParametro} from '../../data/makeDataCrud'; 
import { AuthContext } from "../../mycontext";
import ModalFamiliaArticulo from './modal/ModalFamiliaArticulo';

const URL_BASE=process.env.REACT_APP_URL_BASE;
const API_SEL=process.env.REACT_APP_API_SEL_FAR;
const API_INS=process.env.REACT_APP_API_INS_FAR;
const API_UPD=process.env.REACT_APP_API_UPD_FAR;
const API_DEL=process.env.REACT_APP_API_DEL_FAR;
...
const FamiliaArticulo = () => {
    ...
    const [parametro, setParametro] = useState(inicialParametro);

    const [rowData, setRowData] = useState([]);

    const [inicial, setInicial] = useState(inicialFamiliaArticulo); 
    ...
    const updateSiguienteFamiliaParametro=(idfs,fsiguiente)=>{

      console.log("updateSiguienteFamiliaParametro ",idfs,fsiguiente);  
       ...
       setIsRefetching(true);
         ...
         axios
         .put(`${URL_BASE}${API_UPD_PAR}${idfs}`, {
           data: {
               
            siguientefamiliaarticulo: fsiguiente+1,
           },
         },{    headers:{
         ...
         .then((response) => {
           ...
           setMessage(`Modificado folio en parametro ${fsiguiente}`);
           setOpenSnack(true);
           cargaParametro();
          ...
          axios
              .post(`${URL_BASE}${ API_INS}`, {
                ...
                data: {
                  ...
                  siguientesubfamilia:values.original.siguientesubfamilia
              ...
              .then((response) => {
                ...
                console.log("parametros a modificar==>",parametro.id,
                parametro.siguientefamilia);
                updateSiguienteFamiliaParametro(parametro.id,
                  parametro.siguientefamiliaarticulo);
   ...
   const getSubfamilia_articulos=(laid)=>{
   ...
   const handleDeleteRow = useCallback(
      ...
      (row) => {
        ...
        const re = getSubfamilia_articulos(row.getValue('id'));
 
        console.log("registro a borrar : ",re);

        //existe elemento
        if ( re.subfamilia_articulos){
            console.log("busqueda de id a borrar",re.subfamilia_articulos); 
            console.log("largo", re.subfamilia_articulos );
          
            if  ( re.subfamilia_articulos.length >0){
    ...
    const columns = useMemo(
      () => [
        ...
        {
          accessorKey: 'siguientesubfamilia',
          header: 'siguiente_subfamilia_articulo',
          size: 140,
 ...
 const csvOptions = {
  ...
  const cargaParametro=async()=>{
    ...
    .then(({ data }) => {
      console.log("parametro",data.data[0]);
      setParametro([...data.data][0]);
  ...
  useEffect(() => { 
   ...
   setTitulomod(''); 
   setTitle(inicialFamiliaArticulo.title);
   setSubTitle(inicialFamiliaArticulo.subtitle);
   setIsLoading(true);
   setIsRefetching(true);
            ...
            <Tooltip arrow placement="left" title="Edit">
              <IconButton onClick={() =>{
                 ...
                 const modificar={
                  ...
                  siguientesubfamilia: row.original.siguientesubfamilia
        ...
        renderTopToolbarCustomActions={() => (
          <Button
            ...
            onClick={() => {
              ...
              setInicial(inicialFamiliaArticulo);
...
export default FamiliaArticulo;
